...
export default function DisplayPreview({
  ...
  const frameMax = device === 'mobile' ? 390 : 720

  return (
    <div style={{
      width: frameWidth, maxWidth: frameMax, margin: '0 auto',
      borderRadius: device === 'mobile' ? 28 : 16, overflow: 'hidden',
      border: device === 'mobile' ? '8px solid #1c1b1a' : '1px solid var(--bp-rule)',
      boxShadow: '0 18px 50px rgba(0,0,0,0.18)', background: s.bgColor,
    }}>
      <div className="rsvp-root" style={{ position: 'relative', height: device === 'mobile' ? 620 : 560, overflowY: 'auto', background: s.bgColor }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          {/* Hero */}
          <div style={{
            padding: device === 'mobile' ? '30px 20px 26px' : '44px 32px 34px', textAlign: 'center',
            minHeight: heroImg ? (device === 'mobile' ? 240 : 280) : undefined,
            ...
            <h1 style={{ fontFamily: headingFamily, fontSize: device === 'mobile' ? 'clamp(26px,7vw,34px)' : '42px',
          ...
          {/* Body */}
          <div style={{ padding: device === 'mobile' ? '16px 18px 26px' : '18px 32px 34px', display: 'flex', flexDirection: 'column', gap: 'var(--rsvp-gap, 12px)' }}>
            {[
