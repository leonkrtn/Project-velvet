...
export interface VendorRequestsResult {
  requests: any[]
  isVendor: boolean
}

// Lädt eingehende Marktplatz-Anfragen für den eingeloggten Dienstleister (alle Events),
// angereichert mit Kundendaten (Event-Eckdaten, Gästezahl, Brautpaar-Kontakte).
export async function loadVendorRequests(userId: string): Promise<VendorRequestsResult> {
  ...
  if (dlIds.length === 0) return { requests: [], isVendor: false }
  ...
  return { requests: enriched, isVendor: true }
}
