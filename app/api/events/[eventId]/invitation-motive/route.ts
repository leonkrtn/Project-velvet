...
// POST — presigned R2-Upload-URL für das Einladungs-Motiv.
export async function POST(req: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  const key = `events/${eventId}/invitation-motive`
  const uploadUrl = await requestUploadUrl(key, contentType)
  return NextResponse.json({ uploadUrl, key })
}

// DELETE — Motiv aus R2 entfernen (Key wird separat aus den Settings gelöscht).
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  const { eventId } = await params
  const access = await assertAdmin(eventId)
  if (access.error) return access.error
  await deleteR2Object(`events/${eventId}/invitation-motive`).catch(() => {})
