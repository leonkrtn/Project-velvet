...
export async function POST(
  ...
  for (const row of guestRows) {
    try {
      if (row.id) {
        // Parse route guarantees row.id exists in DB when set → always update
        ...
        // New guest — use admin client to bypass RLS
      ...
      importErrors.push({ rowIndex: row.rowIndex, name: row.name, reason: 'Unerwarteter Fehler' })
    }
  }
    ...
    try {
      ...
      if (!parentGuestId) {
        const { data: existingGuest } = await supabase
          ...
          .single()
        if (existingGuest) parentGuestId = existingGuest.id
      }
      ...
      if (row.id) {
        ...
        const { error: bpUpdateErr } = await supabase
        ...
        // New begleitperson — use admin client to bypass RLS
        const insertObj: Record<string, unknown> = {
        ...
        const { error: bpInsertErr } = await admin
          .from('begleitpersonen')
          .insert(insertObj)
        if (bpInsertErr) {
