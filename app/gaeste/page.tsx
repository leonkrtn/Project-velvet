...
import { type Event, type Guest, type GuestStatus } from "@/lib/store"
...
type Tab = 'anlegen' | 'einladen' | 'liste'
...
const STATUS_BADGE: Record<GuestStatus, GuestStatus> = {
  angelegt:  'angelegt',
  eingeladen:'eingeladen',
  zugesagt:  'zugesagt',
  abgesagt:  'abgesagt',
...
export default function GaestePage() {
  const [tab, setTab]     = useState<Tab>('anlegen')
  ...
  // Liste
  const [statusFilter, setStatusFilter] = useState<GuestStatus | 'alle'>('alle')
...
${event.coupleName}`
  ...
  const copy = async (text: string, id: string) => {
    try { await navigator.clipboard.writeText(text); setCopiedId(id); setTimeout(()=>setCopiedId(null),2000); setToast('Kopiert') } catch {}
  }

  const contactPhone = '1234 567 78910'
  ...
  const letterHtml = (g: Guest) => `
    <div style="font-family:Georgia,serif;background:#fff;padding:48px 40px;max-width:600px;margin:0 auto">
      ...
      <div style="border-top:1px solid #E8E8E8;padding-top:16px">
        <p style="font-size:12px;color:#6B6B6B;line-height:1.7;margin:0 0 8px">Oder ruf uns an — wir nehmen deine Antwort gerne persönlich entgegen:</p>
        <p style="font-family:Georgia,serif;font-size:20px;font-weight:500;color:#C9A84C;margin:0">${contactPhone}</p>
      </div>
  ...
  const printLetters = () => {
    ...
    const guests = toInvite.filter(g => selected.has(g.id))
  ...
  const toInvite   = event.guests.filter(g => g.status === 'angelegt')
  const alreadyInv = event.guests.filter(g => g.status === 'eingeladen')
  const recentGuests = recent.map(id => event.guests.find(g => g.id === id)).filter(Boolean) as Guest[]

  const counts: Record<string,number> = {
    alle:      event.guests.length,
    angelegt:  event.guests.filter(g=>g.status==='angelegt').length,
    eingeladen:event.guests.filter(g=>g.status==='eingeladen').length,
    zugesagt:  event.guests.filter(g=>g.status==='zugesagt').length,
    abgesagt:  event.guests.filter(g=>g.status==='abgesagt').length,
  }

  const filtered = statusFilter === 'alle'
    ? event.guests
    : event.guests.filter(g => g.status === statusFilter)
  ...
  return (
    <PageShell title="Gäste" back="/dashboard">
      ...
      <div style={{display:'flex',gap:4,marginBottom:20,background:'var(--surface)',borderRadius:'var(--r-md)',border:'1px solid var(--border)',padding:4}}>
        {(['anlegen','einladen','liste'] as Tab[]).map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{
            ...
            {t==='anlegen'?'Anlegen':t==='einladen'?'Einladen':'Liste'}
            {t==='einladen'&&toInvite.length>0&&(
      ...
      {tab==='anlegen'&&(
        <div style={{animation:'fadeUp 0.3s ease'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              <div>
                ...
                <input id="gf-name" ref={nameRef} value={name} onChange={e=>setName(e.target.value)}
                  onKeyDown={e=>handleKey('name',e)} placeholder="Vorname Nachname" autoFocus
          ...
          {recentGuests.length > 0 && (
            <>
              <SectionTitle>Gerade angelegt — {recentGuests.length} Gast{recentGuests.length!==1?'e':''}</SectionTitle>
                ...
                {recentGuests.map(g=>(
                  <div key={g.id} style={{display:'flex',alignItems:'center',gap:12,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--r-md)',padding:'12px 14px'}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <p style={{fontSize:11,color:'var(--text-dim)'}}>{[g.email,g.phone].filter(Boolean).join(' · ')||'Keine Kontaktdaten'}</p>
                    </div>
                    <Badge variant="neutral" label="Angelegt"/>
                  </div>
                ))}
              </div>
              <p style={{fontSize:12,color:'var(--text-dim)',textAlign:'center'}}>
                {event.guests.length} Gäste gesamt ·{' '}
                <button onClick={()=>setTab('einladen')} style={{background:'none',border:'none',color:'var(--gold)',fontSize:12,cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>
                  Jetzt einladen →
                </button>
              </p>
            </>
      ...
      {tab==='einladen'&&(
        <div style={{animation:'fadeUp 0.3s ease'}}>
          <p style={{fontSize:13,color:'var(--text-dim)',marginBottom:16,lineHeight:1.6}}>
            Wähle Gäste aus, generiere ihren personalisierten Einladungslink und markiere sie als eingeladen.
          </p>

          {toInvite.length===0?(
            <Card style={{textAlign:'center',padding:'28px 20px'}}>
              <p style={{fontSize:13,color:'var(--text-dim)'}}>Alle angelegten Gäste wurden bereits eingeladen.</p>
              <button onClick={()=>setTab('anlegen')} style={{marginTop:12,background:'none',border:'none',color:'var(--gold)',fontSize:13,cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>
                Neuen Gast anlegen →
              </button>
            </Card>
          ):(
            <>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                <button onClick={()=>{
                  if(selected.size===toInvite.length) setSelected(new Set())
                  else setSelected(new Set(toInvite.map(g=>g.id)))
                }} style={{background:'none',border:'none',fontSize:12,color:'var(--gold)',cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>
                  {selected.size===toInvite.length?'Auswahl aufheben':'Alle auswählen'}
                </button>
                <span style={{fontSize:11,color:'var(--text-dim)'}}>{toInvite.length} noch nicht eingeladen</span>
              </div>

              {/* Personal note for all */}
              <div style={{marginBottom:12}}>
                <label style={labelStyle}>Persönliche Ergänzung (für alle, optional)</label>
              ...
              <div style={{display:'flex',gap:8}}>
                ...
                <Button fullWidth variant="gold" disabled={selected.size===0} onClick={markInvited}>
                  <span style={{display:'flex',alignItems:'center',gap:7,justifyContent:'center'}}>
                    <Mail size={14}/>
                    {selected.size>0?`${selected.size} per Mail einladen`:'Gäste auswählen'}
                  </span>
                </Button>
              </div>
            </>
          ...
          {alreadyInv.length>0&&(
            <div style={{marginTop:24}}>
              <SectionTitle>Bereits eingeladen ({alreadyInv.length})</SectionTitle>
      ...
      {tab==='liste'&&(
        <div style={{animation:'fadeUp 0.3s ease'}}>
          ...
          <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:16}}>
            {(['alle','angelegt','eingeladen','zugesagt','abgesagt'] as const).map(s=>(
              <button key={s} onClick={()=>setStatusFilter(s)} style={{
                ...
                {s==='alle'?'Alle':STATUS_LABELS[s as GuestStatus]}
                <span style={{background:statusFilter===s?'rgba(201,168,76,0.25)':'var(--bg)',borderRadius:100,padding:'1px 7px',fontSize:10}}>
                  {counts[s]}
                </span>
              </button>
            ))}
          ...
          {filtered.length===0&&(
            <p style={{fontSize:13,color:'var(--text-dim)',textAlign:'center',padding:'28px 0'}}>Keine Gäste in dieser Kategorie.</p>
          )}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:7}}>
            {filtered.map(g=>{
              ...
              return (
                <div key={g.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--r-md)',overflow:'hidden'}}>
                  <div style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',cursor:'pointer'}} onClick={()=>setExpanded(isExp?null:g.id)}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
                      <Badge variant={STATUS_BADGE[g.status]} label={STATUS_LABELS[g.status]}/>
                  ...
                  {isExp&&(
                    <div style={{borderTop:'1px solid var(--border)',padding:'14px',background:'var(--bg)'}}>
                      {g.address&&(
                        <InfoRow label="Adresse" value={g.address}/>
                      )}
                      {g.trinkAlkohol!==undefined&&(
                        <InfoRow label="Alkohol" value={g.trinkAlkohol?'Ja':'Nein'}/>
                      )}
                      {g.meal&&(
                        <InfoRow label="Menü" value={MEAL_LABELS[g.meal]}/>
                      )}
                        ...
                        <div style={{marginBottom:10}}>
                          ...
                          {g.begleitpersonen.map(bp=>(
                            <div key={bp.id} style={{display:'flex',gap:8,alignItems:'center',marginBottom:4}}>
                              ...
                              {bp.trinkAlkohol!==undefined&&(
                                <>
                                  <span style={{fontSize:10,color:'var(--text-dim)'}}>·</span>
                                  <p style={{fontSize:11,color:'var(--text-dim)'}}>{bp.trinkAlkohol?'Alkohol: Ja':'Alkohol: Nein'}</p>
                                </>
                              )}
                            </div>
                          ))}
