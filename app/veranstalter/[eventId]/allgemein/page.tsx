...
export default async function AllgemeinPage({ params }: Props) {
  ...
  // Load brautpaar members for contact section
  const { data: bpMembers } = await supabase
    .from('event_members')
  ...
  return <AllgemeinForm eventId={eventId} initialData={event} bpMembers={bpNormalized} />
}
