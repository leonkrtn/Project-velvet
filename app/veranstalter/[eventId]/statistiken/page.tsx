...
export default async function StatistikenPage({ params }: Props) {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.5px', marginBottom: 6 }}>Statistiken</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 24, marginBottom: 24 }}>
        ...
        <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 24 }}>
          ...
          {vendorTotal === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--text-tertiary)', fontStyle: 'italic' }}>Keine Daten</p>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
              <svg width={160} height={160} style={{ flexShrink: 0 }}>
                <circle cx={80} cy={80} r={DONUT_R} fill="none" stroke="#F0F0F2" strokeWidth={20} />
                {donutSegments.map(s => (
                  <circle
                    key={s.cat}
                ...
                <text x={80} y={75} textAnchor="middle" style={{ fontSize: 11, fill: 'var(--text-tertiary)' }}>Gesamt</text>
                <text x={80} y={93} textAnchor="middle" style={{ fontSize: 13, fontWeight: 700, fill: 'var(--text-primary)' }}>{fmtMoney(vendorTotal)}</text>
              </svg>
              <div style={{ flex: 1 }}>
                {donutSegments.map(s => (
                  <div key={s.cat} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                    <div style={{ width: 10, height: 10, borderRadius: 2, background: s.color, flexShrink: 0 }} />
                    <span style={{ fontSize: 12, flex: 1, color: 'var(--text-primary)' }}>{s.cat}</span>
        ...
        {/* Gäste-Status */}
        <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 24 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 20 }}>Gäste-Status</h3>
          ...
          {guests.length > 0 && (
            <div>
              <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginBottom: 6 }}>Rücklauf</div>
              <div style={{ height: 8, background: '#F0F0F2', borderRadius: 4, overflow: 'hidden', display: 'flex' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
                {Object.entries(guestByStatus).map(([status, count]) => (
                  <span key={status} style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
      ...
      {vendorCategories.length > 0 && (
        <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 24, marginBottom: 24 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 20 }}>Ausgaben nach Kategorie</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {vendorCategories.map(([cat, val], i) => (
              <div key={cat} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 120, fontSize: 13, color: 'var(--text-primary)', flexShrink: 0 }}>{cat}</div>
                <div style={{ flex: 1, height: 20, background: '#F0F0F2', borderRadius: 4, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 24 }}>
        ...
        <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 24 }}>
          ...
          {vendors.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--text-tertiary)', fontStyle: 'italic' }}>Keine Dienstleister</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  <th style={{ textAlign: 'left', padding: '6px 8px', fontWeight: 600, color: 'var(--text-tertiary)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Name</th>
                  <th style={{ textAlign: 'left', padding: '6px 8px', fontWeight: 600, color: 'var(--text-tertiary)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Kategorie</th>
                  <th style={{ textAlign: 'right', padding: '6px 8px', fontWeight: 600, color: 'var(--text-tertiary)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Kosten</th>
                </tr>
              </thead>
              <tbody>
                {vendors.map(v => (
                  <tr key={v.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '8px 8px', color: 'var(--text-primary)' }}>{v.name ?? '—'}</td>
                    <td style={{ padding: '8px 8px', color: 'var(--text-tertiary)' }}>{v.category ?? '—'}</td>
        ...
        {/* Tasks progress */}
        <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 24 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 16 }}>Aufgaben-Fortschritt</h3>
          {tasks.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--text-tertiary)', fontStyle: 'italic' }}>Keine Aufgaben</p>
          ) : (
            <>
              ...
              <div style={{ height: 8, background: '#F0F0F2', borderRadius: 4, overflow: 'hidden', marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {tasks.slice(0, 8).map(t => (
                  <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                    ...
                    <span style={{ color: t.done ? 'var(--text-tertiary)' : 'var(--text-primary)', textDecoration: t.done ? 'line-through' : 'none' }}>
                      {t.title ?? 'Aufgabe'}
                    </span>
                  </div>
                ))}
...
function KpiBox({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: '18px 20px' }}>
      <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-tertiary)', marginBottom: 8 }}>{label}</div>
...
function GuestStat({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div style={{ padding: '12px 14px', background: '#F0F0F2', borderRadius: 'var(--radius-sm)' }}>
