...
export function normalizePool(raw: unknown): RaumTablePool {
...
export type SitzplanInitialConfig = { points: RaumPoint[]; elements: RaumElement[]; table_pool: unknown } | null

interface Props {
  eventId: string
  userId: string
  coupleName: string
  globalPoints: RaumPoint[]
  globalElements: RaumElement[]
  initialEventConfig: SitzplanInitialConfig
  initialPlacedTables: PlacedTablePreview[]
  initialConcepts: SeatingConcept[]
}

export default function SitzplanClient({
  eventId,
  userId,
  coupleName,
  globalPoints,
  ...
  // Room config
  const [eventPoints,    setEventPoints]    = useState<RaumPoint[] | null>(initialEventConfig ? (initialEventConfig.points ?? []) : null)
  const [eventElements,  setEventElements]  = useState<RaumElement[] | null>(initialEventConfig ? (initialEventConfig.elements ?? []) : null)
  const [eventTablePool, setEventTablePool] = useState<RaumTablePool | null>(initialEventConfig ? normalizePool(initialEventConfig.table_pool) : null)
  const [hasEventConfig, setHasEventConfig] = useState(!!initialEventConfig)

  // Placed tables (for overlay in RaumKonfigurator)
  const [placedTables, setPlacedTables] = useState<PlacedTablePreview[]>(initialPlacedTables)
  ...
  // Seating concepts
  const [concepts] = useState<SeatingConcept[]>(initialConcepts)
