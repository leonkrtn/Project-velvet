...
// ── Main page ─────────────────────────────────────────────────────────────────
export interface PersonalplanungInitialData {
  staff: StaffMember[]
  days: PlanDay[]
...
export default function PersonalplanungClient({ eventId, initial }: { eventId: string; initial: PersonalplanungInitialData }) {
  ...
  const [activeDayId, setActiveDayId] = useState<string | null>(initial.days[0]?.id ?? null)
  const [eventInfo, setEventInfo] = useState<EventInfo | null>(initial.eventInfo)
  const [timelineEntries, setTimelineEntries] = useState<TimelineEntry[]>(initial.timelineEntries)
  ...
  // Swap requests
  const [swaps, setSwaps] = useState<Swap[]>(initial.swaps)
  const [showSwapsPanel, setShowSwapsPanel] = useState(false)

  // Monthly time logs
  const [timeLogs, setTimeLogs] = useState<ShiftTimeLog[]>(initial.timeLogs)
