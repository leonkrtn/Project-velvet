...
export default function OnboardingPage() {
  ...
  const [hotelRooms,setHotelRooms] = useState<HotelRoom[]>([{id:uid(),type:'Doppelzimmer',totalRooms:10,bookedRooms:0,pricePerNight:120}])
  // Step 5
  ...
  const finish = async () => {
    ...
    const ev: Event = {
      ...
      hotelName: hotelName.trim(), hotelAddress: hotelAddress.trim(),
      hotelRooms: hotelRooms.filter(r=>r.type.trim()),
      mealOptions,
      ...
      guests: guestList.filter(g=>g.name.trim()&&g.email.trim()).map(g=>({
  ...
  return (
    <div style={{background:'var(--bg)',minHeight:'100dvh',paddingBottom:40}}>
      ...
      <div style={{maxWidth:600,margin:'0 auto',padding:'24px 20px'}}>
        ...
        {step===4&&(
          <div style={{animation:'fadeUp 0.4s ease'}}>
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:24,fontWeight:400,color:'var(--text)',marginBottom:24}}>Hotel & Zimmerkontingent</h2>
            <Block>
              <Lbl>Name des Hotels</Lbl>
              <input value={hotelName} onChange={e=>setHotelName(e.target.value)} placeholder="z.B. Schlosshotel Neuhof" style={{...IS,marginBottom:14}} onFocus={focusGold} onBlur={blurBorder}/>
              <Lbl>Adresse des Hotels</Lbl>
              <input value={hotelAddress} onChange={e=>setHotelAddress(e.target.value)} placeholder="Straße, PLZ, Ort" style={IS} onFocus={focusGold} onBlur={blurBorder}/>
            </Block>
            <Block mb={0}>
              <SecTitle>Zimmerkontingent</SecTitle>
              <SecSub>Welche Zimmer könnt ihr für eure Gäste reservieren?</SecSub>
              <div style={{display:'flex',flexDirection:'column',gap:10}}>
                {hotelRooms.map((room,i)=>(
                  <div key={room.id} style={{background:'var(--bg)',borderRadius:12,padding:'14px'}}>
                    <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:10}}>
                      <input value={room.type} onChange={e=>{const a=[...hotelRooms];a[i]={...a[i],type:e.target.value};setHotelRooms(a)}} placeholder="Zimmertyp (z.B. Doppelzimmer)" style={{...IS,marginBottom:0,flex:1}} onFocus={focusGold} onBlur={blurBorder}/>
                      {hotelRooms.length>1&&<button onClick={()=>setHotelRooms(hotelRooms.filter((_,j)=>j!==i))} style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-dim)',padding:4}}><Trash2 size={14}/></button>}
                    </div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                      <div><Lbl>Anzahl Zimmer</Lbl><input type="number" value={room.totalRooms} onChange={e=>{const a=[...hotelRooms];a[i]={...a[i],totalRooms:Number(e.target.value)};setHotelRooms(a)}} style={{...IS,marginBottom:0}} onFocus={focusGold} onBlur={blurBorder}/></div>
                      <div><Lbl>Preis / Nacht (€)</Lbl><input type="number" value={room.pricePerNight} onChange={e=>{const a=[...hotelRooms];a[i]={...a[i],pricePerNight:Number(e.target.value)};setHotelRooms(a)}} style={{...IS,marginBottom:0}} onFocus={focusGold} onBlur={blurBorder}/></div>
                    </div>
                  </div>
                ))}
                <button onClick={()=>setHotelRooms([...hotelRooms,{id:uid(),type:'',totalRooms:0,bookedRooms:0,pricePerNight:0}])} style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',borderRadius:100,border:'1px dashed var(--border)',background:'none',fontSize:12,fontWeight:600,color:'var(--text-dim)',cursor:'pointer',fontFamily:'inherit',alignSelf:'flex-start'}}>
        ...
        <div style={{display:'flex',gap:12,marginTop:32,justifyContent:'space-between'}}>
          ...
          <button onClick={next} style={{display:'flex',alignItems:'center',gap:6,padding:'12px 24px',borderRadius:100,border:'none',background:canNext()?'var(--gold)':'var(--border)',color:canNext()?'#FFFFFF':'var(--text-dim)',fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:'inherit',transition:'background 0.2s'}}>
            {step===STEPS.length?(<><Check size={15}/> Fertigstellen</>):(<>Weiter <ChevronRight size={15}/></>)}
          </button>
        </div>
      </div>
